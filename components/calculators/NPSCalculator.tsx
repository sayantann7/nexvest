...
import { calculateNps, buildNpsChart } from "@/features/calculators/logic/nps";
      ...
      <div className="space-y-4">
        <Card className="bg-[#131740] border-white/10 text-white">
          <CardContent className="p-6 space-y-2">
            ...
            <div className="flex items-center justify-between">
              <span className="text-slate-300">Estimated Corpus</span>
              <span className="font-semibold">{currency(result.corpus)}</span>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-slate-300">60% Lumpsum</span>
              <span>{currency(result.lumpsum)}</span>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-slate-300">40% Annuity</span>
              <span>{currency(result.annuitized)}</span>
            </div>
          </CardContent>
        </Card>
        <Card className="bg-[#131740] border-white/10 text-white">
                ...
                <AreaChart data={chartData} margin={{ left: 6, right: 12, top: 10, bottom: 0 }}>
                  ...
                  <Tooltip
                    ...
                    formatter={(v: number | string) =>
                      currency((Number(v) / 1000) * (result.investedAmount / (years * 12)))
                    }
                    contentStyle={{
                      background: "#111536",
