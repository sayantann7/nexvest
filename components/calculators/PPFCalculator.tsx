...
// PPF approximation: yearly deposit at fixed rate r compounded yearly for 15 years: FV = P * [((1+r)^n -1)/r] * (1+r)
  ...
  return (
    <div className="grid gap-6 md:grid-cols-2">
      <div className="space-y-4">
        <div>
          <label className="block mb-1 text-sm text-slate-300">Yearly contribution (₹)</label>
          <Input type="number" value={yearly} onChange={(e)=>setYearly(Math.max(0, Number(e.target.value)))} className="bg-[#131740] border-white/10 text-white" />
        </div>
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block mb-1 text-sm text-slate-300">Interest rate %</label>
      ...
      <div className="space-y-4">
        <Card className="bg-[#131740] border-white/10 text-white">
          <CardContent className="p-6 space-y-2">
            ...
            <div className="flex items-center justify-between"><span className="text-slate-300">Maturity Value</span><span className="font-semibold">{currency(result.fv)}</span></div>
            ...
            <div className="h-60">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={chartData} margin={{ left: 6, right: 12, top: 10, bottom: 0 }}>
                  ...
                  <Legend />
                  <Area type="monotone" dataKey="invested" name="Invested" stroke="#09ffec" fill="url(#ppfA)" />
                  <Area type="monotone" dataKey="value" name="Value" stroke="#8b5cf6" fill="url(#ppfB)" />
                </AreaChart>
              </ResponsiveContainer>
            </div>
          </CardContent>
