...
// Compound interest: A = P*(1 + r/m)^(m*n)
export default function CompoundInterestCalculator() {
  ...
  const result = useMemo(() => {
    const A = principal * Math.pow(1 + rate/100/m, m*years);
  ...
  const chartData = useMemo(() => {
    ...
    for (let y=1; y<=years; y++) {
      const A = principal * Math.pow(1 + r, m*y);
    ...
    <div className="grid gap-6 md:grid-cols-2">
      <div className="space-y-4">
        ...
        <div>
          ...
          <Input type="number" value={m} onChange={(e)=>setM(Number(e.target.value))} className="bg-[#131740] border-white/10 text-white" />
        </div>
