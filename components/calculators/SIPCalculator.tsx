...
import { calculateSip } from "@/features/calculators/logic/sip";
...
export default function SIPCalculator() {
  ...
  const chartData = useMemo(() => {
    ...
    for (let m = 1; m <= months; m++) {
      // deposit at month start, then grow
  ...
  return (
    <div className="grid gap-6 md:grid-cols-2">
      <div className="space-y-4">
        <div>
          <label className="block mb-1 text-sm text-slate-300">Monthly investment (₹)</label>
          <Input
            type="number"
            value={monthly}
        ...
        <Button onClick={() => {}} className="bg-[#09ffec] text-[#0D0C34] hover:bg-[#09ffec]/90">
          Calculate
        </Button>
      </div>
      <div className="space-y-4">
        <Card className="bg-[#131740] border-white/10 text-white">
          <CardContent className="p-6 space-y-2">
            ...
            <div className="flex items-center justify-between">
              ...
              <span className="font-semibold">{currency(result.futureValue)}</span>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-slate-300">Wealth gain</span>
              <span className="text-emerald-400">{currency(result.gain)}</span>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-slate-300">Implied CAGR</span>
              <span>{(result.impliedCagr * 100).toFixed(2)}%</span>
            </div>
          </CardContent>
        </Card>
        <Card className="bg-[#131740] border-white/10 text-white">
