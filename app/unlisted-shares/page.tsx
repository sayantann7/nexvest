...
import type { UnlistedShare } from "@/features/unlisted-shares/types";
import { getUnlistedShares } from "@/features/unlisted-shares/api";
...
export default function UnlistedShares() {
  ...
  const [filteredShares, setFilteredShares] = useState<UnlistedShare[]>([]);
  ...
  useEffect(() => {
    async function fetchShares() {
      try {
        const data = await getUnlistedShares();
        setShares(data);
            ...
            <div className="grid lg:grid-cols-2 gap-10 items-center">
              <div>
                ...
                <p className="text-lg text-slate-300 mb-6 max-w-xl">
                  Private Equity That Powers India&apos;s Next Decade. NexVest backs bold Indian
                  founders, disruptive ideas and high-growth sectors, connecting visionary capital
                  with transformative businesses. We don&apos;t just invest; we partner to scale and
                  create impact.
                </p>
                <div className="flex flex-wrap gap-3 mb-8">
                  <div className="px-3 py-2 rounded-xl border border-white/10 bg-[#131740] text-sm text-slate-300 flex items-center gap-2">
                    <CheckCircle2 className="w-4 h-4 text-emerald-400" />
                    T+1–T+3 settlement
                  </div>
                  <div className="px-3 py-2 rounded-xl border border-white/10 bg-[#131740] text-sm text-slate-300 flex items-center gap-2">
                    <Shield className="w-4 h-4 text-[#09ffec]" />
                    KYC verified
                  </div>
                  <div className="px-3 py-2 rounded-xl border border-white/10 bg-[#131740] text-sm text-slate-300 flex items-center gap-2">
                ...
                <a
                  href="#browse"
                  className="inline-flex items-center gap-2 bg-[#09ffec] text-[#0D0C34] hover:bg-[#09ffec]/90 font-semibold rounded-xl px-5 py-3 transition-colors"
                >
        ...
        <section className="container mx-auto px-4 py-12 md:py-16">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            <div className="bg-[#131740] border border-white/10 rounded-2xl p-6 text-center">
              <div className="w-10 h-10 rounded-full bg-emerald-500/20 text-emerald-300 flex items-center justify-center mx-auto font-semibold mb-4">
                1
              </div>
          ...
          <div className="mb-8">
            ...
            <div className="grid lg:grid-cols-2 gap-6">
              <div className="bg-[#131740] border border-white/10 rounded-2xl p-6">
                ...
                <p className="text-slate-300">
                  Equity of private companies not traded on exchanges. Accessed via private deals,
                  ESOPs, or pre-IPO rounds. Pricing is negotiated; liquidity is limited.
                </p>
              </div>
              <div className="bg-[#131740] border border-white/10 rounded-2xl p-6">
                ...
                <ul className="space-y-2 text-slate-300">
                  <li className="flex items-start gap-2">
                    <CheckCircle2 className="w-5 h-5 text-emerald-400 mt-0.5" /> Settlement:
                    typically T+1–T+3
                  </li>
                  <li className="flex items-start gap-2">
                    <CheckCircle2 className="w-5 h-5 text-emerald-400 mt-0.5" /> KYC: PAN, Aadhaar,
                    Demat required
                  </li>
                  <li className="flex items-start gap-2">
                    <CheckCircle2 className="w-5 h-5 text-emerald-400 mt-0.5" /> Pricing: negotiated
                    quotes (demand/supply)
                  </li>
                  <li className="flex items-start gap-2">
                    <CheckCircle2 className="w-5 h-5 text-emerald-400 mt-0.5" /> Liquidity: exits
                    depend on counterparty interest
                  </li>
                </ul>
              </div>
            </div>
          ...
          <div className="grid md:grid-cols-2 gap-6 mb-8">
            <div className="bg-[#131740] border border-white/10 rounded-2xl p-6">
              <h3 className="font-heading text-xl font-semibold mb-3 flex items-center gap-2">
                <TrendingUp className="w-5 h-5 text-[#09ffec]" />
                Key Benefits
              </h3>
              <ul className="space-y-2 text-slate-300">
                <li className="flex items-start gap-2">
                  <CheckCircle2 className="w-5 h-5 text-emerald-400 mt-0.5" /> Early access to
                  promising companies (pre-IPO)
                </li>
                <li className="flex items-start gap-2">
                  <CheckCircle2 className="w-5 h-5 text-emerald-400 mt-0.5" /> Potential valuation
                  upside post listing/events
                </li>
                <li className="flex items-start gap-2">
                  <CheckCircle2 className="w-5 h-5 text-emerald-400 mt-0.5" /> Diversification
            ...
            <div className="bg-[#131740] border border-white/10 rounded-2xl p-6">
              ...
              <ul className="space-y-2 text-slate-300">
                <li className="flex items-start gap-2">
                  <AlertTriangle className="w-5 h-5 text-rose-400 mt-0.5" /> Lower liquidity; exits
                  may take time/discounts
                </li>
                <li className="flex items-start gap-2">
                  <AlertTriangle className="w-5 h-5 text-rose-400 mt-0.5" /> Fewer disclosures;
                  diligence is essential
                </li>
                <li className="flex items-start gap-2">
                  <AlertTriangle className="w-5 h-5 text-rose-400 mt-0.5" /> Negotiated prices;
                  spreads can be wide
                </li>
              </ul>
            </div>
              ...
              {[
                {
                  ...
                  a: "Prices are discovered via negotiated quotes based on recent deals, demand/supply, company performance, and expected listing timelines.",
                },
                {
                  q: "What is the typical settlement time?",
                  a: "T+1 to T+3 in most cases, depending on the transfer agent and DP. Certain operational constraints can extend timelines.",
                },
                {
                  q: "Are unlisted shares riskier than listed ones?",
                  a: "They can be due to lower liquidity and fewer disclosures. Diversify appropriately and do adequate diligence.",
                },
                {
                  q: "What documents do I need?",
                  a: "Standard KYC (PAN, Aadhaar, address), Demat details, and bank proof. Additional documents may be requested by counterparties.",
                },
                {
                  q: "Can I sell anytime?",
                  a: "Liquidity depends on demand. Exit timelines may vary, and you may need to accept discounts versus indicated offers.",
                },
                {
                  q: "Do corporate actions affect my holdings?",
                  a: "Yes. Corporate actions, lock-ins, and ISIN changes can affect transferability and timelines.",
                },
              ].map((item, idx) => (
                <details
                  key={idx}
                  className="group rounded-xl border border-white/10 bg-[#131740] open:bg-[#141a4a] transition-colors"
                >
                    ...
                    <div className="p-6">
                      <div className="flex items-start">
                        ...
                        <div className="ml-4 flex-1">
                          <h3 className="font-heading font-semibold text-lg mb-2 text-white line-clamp-2">
                            {share.title}
                          </h3>
                        </div>
                      </div>
