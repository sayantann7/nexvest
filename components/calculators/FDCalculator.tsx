...
// FD (compounded quarterly by default): A = P*(1 + r/m)^(m*n)
export default function FDCalculator() {
  const [principal, setPrincipal] = useState(500000);
  const [rate, setRate] = useState(7.5);
  const [years, setYears] = useState(5);
  const [compounding, setCompounding] = useState(4); // times per year

  const result = useMemo(() => {
    const r = rate / 100;
    const A = principal * Math.pow(1 + r / compounding, compounding * years);
  ...
  const chartData = useMemo(() => {
    ...
    for (let y=1; y<=years; y++) {
      const A = principal * Math.pow(1 + r / compounding, compounding * y);
  ...
  return (
    <div className="grid gap-6 md:grid-cols-2">
      <div className="space-y-4">
        <div>
          <label className="block mb-1 text-sm text-slate-300">Deposit amount (₹)</label>
        ...
        <div>
          <label className="block mb-1 text-sm text-slate-300">Compounding (per year)</label>
          <Input type="number" value={compounding} onChange={(e)=>setCompounding(Number(e.target.value))} className="bg-[#131740] border-white/10 text-white" />
        </div>
      ...
      <div className="space-y-4">
        ...
        <Card className="bg-[#131740] border-white/10 text-white">
          <CardContent className="p-4">
            <div className="text-sm text-slate-300 mb-3">Maturity growth</div>
            <div className="h-60">
